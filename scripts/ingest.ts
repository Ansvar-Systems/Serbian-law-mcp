#!/usr/bin/env tsx
/**
 * Serbian Law MCP -- Ingestion Pipeline
 *
 * Fetches Serbian legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Serbian Parliament) provides free public access to all legislation
 ...
 * License: Serbian legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseSerbianHtml, KEY_SERBIAN_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Serbian Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseSerbianHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Serbian Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Serbian Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
  ...
  const acts = limit ? KEY_SERBIAN_ACTS.slice(0, limit) : KEY_SERBIAN_ACTS;
