#!/usr/bin/env tsx
/**
 * Serbian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.pravno-informacioni-sistem.rs';
const PORTAL_NAME = 'Pravno-informacioni sistem';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/serbian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Serbian Law MCP — Data Freshness Check');
