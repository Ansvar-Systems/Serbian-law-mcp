/**
 * search_eu_implementations — Search EU directives/regulations referenced by Serbian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_serbian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as serbian_statute_count
  ...
  if (input.has_serbian_implementation) {
    sql += ' HAVING serbian_statute_count > 0';
  }
