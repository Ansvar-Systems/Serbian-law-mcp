/**
 * get_serbian_implementations — Find Serbian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetSerbianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SerbianImplementationResult {
  document_id: string;
...
export async function getSerbianImplementations(
  db: InstanceType<typeof Database>,
  input: GetSerbianImplementationsInput,
): Promise<ToolResponse<SerbianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SerbianImplementationResult[];
