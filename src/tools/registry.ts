/**
 * Tool registry for Serbian Law MCP Server.
...
import { getSerbianImplementations, type GetSerbianImplementationsInput } from './get-serbian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Official Gazette of the Republic of Serbia. ' +
    ...
    'Call this FIRST when you need to understand what Serbian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Serbian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Serbian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Serbian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Serbian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Serbian?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Serbian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Serbian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that a Serbian statute references or aligns with. ' +
      'Serbian is an EU candidate state aligning its legislation with the EU acquis ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Serbian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_serbian_implementations',
    description:
      'Find all Serbian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Serbian statutes. ' +
      'Serbian transposes EU directives into national law as an EU candidate state.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Serbian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_serbian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Serbian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Serbian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Serbian statute or provision. ' +
      ...
      'Serbian is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Serbian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_serbian_implementations':
          result = await getSerbianImplementations(db, args as unknown as GetSerbianImplementationsInput);
          break;
